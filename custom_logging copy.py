...
class CustomTrackingCallback(BaseCallback):
    ...
    def __init__(self, check_freq: int, monitor_dir: str, start_time: float, verbose=1):
        super(CustomTrackingCallback, self).__init__(verbose)
        self.check_freq = check_freq
        self.monitor_dir = monitor_dir
        self.start_time = start_time
        # self.save_path = monitor_dir
        # self.checkpoint_save_path = os.path.join(self.logger.dir, "checkpoint_model")
        # self.periodic_save_path = os.path.join(self.logger.dir, "periodic_model")
        ...
        if self.n_calls % self.check_freq == 0:
            ...
            if len(x) > 0:
                ...
                if mean_reward > self.best_mean_reward:
                    ...
                    if self.verbose > 0:
                        print("Saving new best model to {}".format(self.logger.dir))
                    self.model.save(f"{self.logger.dir}/best")
                    open(f"{self.logger.dir}/best.zip").close()

                if mean_reward < self.worst_mean_reward:
                    self.worst_mean_reward = mean_reward
                    if self.verbose > 0:
                        print("Saving new worst model to {}".format(self.logger.dir))
                    self.model.save(f"{self.logger.dir}/worst")
                    open(f"{self.logger.dir}/worst.zip").close()
